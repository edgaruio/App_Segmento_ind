...
mes_actual <- as.Date.character("2020-05-01")
...
df_bono <- bono %>% 
  filter(AÑO == year(mes_actual) & MES == month(mes_actual)) %>% 
...
df_preaprobados <- sqlQuery( 
  ...
  filter(fecha_vigencia >= mes_actual - 30) %>% 
...
df_derecho_cm <- sqlQuery(
  ...
  mutate(año = as.numeric(año),
         mes = as.numeric(mes)) %>% 
  filter(año == year(mes_actual)) %>% 
  filter(mes == month(mes_actual)) %>% 
  select(id_persona) %>% 
  distinct() %>% 
...
### cuota redimida ====
con_redencion <- odbcDriverConnect("Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=//bogak08beimrodc/bi/Consumo/Convenios/ConsumoConvenios_Mayo.accdb")
sqlTables(con_redencion)

redencion_tms <- sqlQuery(
  con_redencion, 
  ...
  dplyr::select(tarjeta, valor_transaccion)
str(redencion_tms)

odbcCloseAll()

df_redimida_cm <- redencion_tms %>%
  left_join(df_persona_tms, by = c("tarjeta"="numero_tarjeta_multiservicios")) %>%
  # filter(FechaTransaccion >= mes_actual & FechaTransaccion <= mes_actual + 30) %>%
  # filter(TipoTransaccion == "1") %>%
  select(id_persona, valor_cm = valor_transaccion) %>%
  group_by(id_persona) %>%
  ...
  data.frame()
str(df_redimida_cm)
table(duplicated(df_redimida_cm$id_persona))
    
df_cuotas <- df_derecho_cm %>% 
  left_join(df_redimida_cm, by = c("id_persona"))
str(df_cuotas)
rm(df_derecho_cm,df_redimida_cm,redencion_tms)
...
consumo_individual <- readRDS("//Bogak08beimrodc/bi/Analitica/ConsumosRDS/5 - Mayo/consumo_individual_Mayo2020.rds") %>% 
  ...
  filter(anno == max(anno, na.rm = T)) %>% 
  filter(mes == max(mes, na.rm = T)) %>% 
  data.frame()
...
#          Hotel = `Hotel Alcaravan` + `Hotel Bosques De Athan` + `Hotel Penalisa` + `Hotel Colonial` + `Hotel Lanceros`
#          ) %>%
#   select(-c(`Club Bellavista`,`Club Calle 195`,`Club El Cubo`,`Club La Colina`,
#             `Hotel Alcaravan`,`Hotel Bosques De Athan`,`Hotel Penalisa`, `Hotel Colonial`, `Hotel Lanceros`,
...
# library(purrr)
saveRDS(bd_afiliados, file = "App/Data/bd_afiliados1_23062020.rds")
saveRDS(bd_afiliados, file = "Data/Originales/Mayo2020/bd_afiliados1_23062020.rds")
str(bd_afiliados)
